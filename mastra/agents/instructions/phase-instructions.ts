...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    recommend: [
      ...
      `Platform: ${platformType} | Workflow: ${workflowName}`,
      "",
      "IMMEDIATELY call getOutcomes({ platformType: '" + platformType + "' })",
      "After tool completes, say: 'Choose your goal'",
      "",
      "If user asks 'help decide': Give 2-sentence pro/con for each, then ask them to choose.",
      "When user selects (__ACTION__:select_outcome:X): Confirm and proceed.",
    ].join("\n"),
    ...
    style: [
      ...
      "",
      "## Behavior rules",
      "- If browsing (no __ACTION__), the tool will display style pairs automatically",
