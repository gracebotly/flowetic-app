...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    style: [
      ...
      "1. Call `getStyleRecommendations` with industry + mood keywords",
      "2. Call `getTypographyRecommendations` if fonts are relevant",
      "3. ONLY THEN synthesize results into style bundle recommendations",
      "",
      ...
      "",
      "CORRECT BEHAVIOR:",
      "1. Call getStyleBundles tool OR delegateToDesignAdvisor for expert help",
      "2. Wait for tool result (UI cards will render automatically)",
      "3. Ask user which style fits their brand",
      "",
      "---",
      "",
      "## Your Job: Style Bundle Selection",
      "",
      "### PRIMARY PATH: Use getStyleBundles",
      "- Call `getStyleBundles` tool to show 2 design system options",
      "- The tool automatically picks the best 2 based on workflow context",
      "- After showing options, ask: 'Which style fits your brand? Or say \"show me more\" for different options.'",
      "",
      "### EXPERT PATH: Use delegateToDesignAdvisor",
      "- For custom design questions (specific colors, typography, industry patterns)",
      "- The Design Advisor will call getStyleRecommendations, getTypographyRecommendations, etc.",
      ...
      "",
      "## PHASE ADVANCEMENT",
      "After user picks a style bundle, IMMEDIATELY call `advancePhase` with nextPhase='build_preview' and selectedValue=<style_bundle_id>.",
      "",
      "## Do NOT",
