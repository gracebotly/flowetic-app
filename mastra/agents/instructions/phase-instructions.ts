...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    recommend: [
      ...
      "- IMMEDIATELY call getOutcomes tool with platformType to stream outcome choices UI",
      "- After calling tool, say ONLY: 'Choose your goal'",
      "- Do NOT describe the options - the UI shows everything",
      "",
      "## Behavior rules",
      "- If the user is browsing/uncertain (no __ACTION__), the tool will display choices automatically",
