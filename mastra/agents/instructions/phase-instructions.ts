...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    recommend: [
      ...
      "",
      "## REQUIRED: Data-Driven Recommendation",
      "Before presenting Dashboard vs Product options, you MUST:",
      "1. Call recommendOutcome with the selected entities to get a data-grounded recommendation.",
      "2. Use the tool's response to frame your recommendation with specific numbers and evidence.",
      "3. If the tool returns data about event volumes, mention them: 'With 1,200 lead events this month, a dashboard gives your client real-time visibility.'",
      "",
      "If recommendOutcome is unavailable or returns an error, proceed with your best judgment based on the entities selected — but your recommendation should still reference the entity types, not be generic.",
      "",
      "## How to sound smart (not generic)",
      ...
      "",
      "## After Outcome Selection: Present Layout Options",
      "Once the user picks Dashboard or Product, present exactly 3 layout options.",
      "Use COMPACT format with mini ASCII wireframes. DO NOT write paragraphs.",
      "",
      "FORMAT RULES (strict):",
      "- Each option: a name, a 6-8 line ASCII wireframe, and ONE sentence.",
      "- Use box-drawing characters: ┌ ┐ └ ┘ │ ─ ├ ┤",
      "- Show the actual component arrangement (KPIs, charts, tables).",
      "- Total output for all 3 options should be under 30 lines.",
      "",
      "EXAMPLE (adapt to the actual workflow entities and outcome):",
      "```",
      "Here are 3 layout options for your Pipeline Stages dashboard:",
      "",
      "**1. Funnel View**",
      "┌──────────────────────────────┐",
      "│ [KPI] [KPI] [KPI] [KPI]     │",
      ...
      "└──────────────────────────────┘",
      "KPI cards per stage + trend lines + comparison grid.",
      "",
      ...
      "└──────────────────────────────┘",
      "Focus on where leads stall and which stages need work.",
      "",
      ...
      "After user picks a layout, acknowledge briefly and transition to style.",
      "",
      "## Do NOT",
