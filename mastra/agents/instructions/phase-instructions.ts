...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    recommend: [
      ...
      "",
      "## PHASE ADVANCEMENT",
      "When user picks Dashboard or Product, call `advancePhase` with nextPhase='style' and selectedValue=<dashboard_or_product>.",
      "After layout selection, you're already in style phase.",
      "",
      "## Behavior rules",
    ...
    style: [
      ...
      "For expert design recommendations, call `delegateToDesignAdvisor`",
      "For style bundle selection, use `getStyleBundles` tool directly",
