...
export function getPhaseInstructions(phase: FloweticPhase, ctx: PhaseInstructionContext): string {
  ...
  const templates: Record<FloweticPhase, string> = {
    ...
    select_entity: [
      ...
      "",
      "## PHASE ADVANCEMENT",
      "After user picks entities, call `advancePhase` with nextPhase='recommend' and selectedEntities=<comma_separated_entity_names>.",
      "",
    ...
    // ─────────────────────────────────────────────────────────────────────────
    style: [
      "# STRICT OUTPUT RULES FOR PHASE 3",
      "DO NOT generate any text, wireframes, or ASCII art before calling design tools.",
      "DO NOT describe styles in text - use the design workflow/tools.",
      ...
      "",
      "## CRITICAL: Design System Generation",
      "When a user needs dashboard styling, you have two options:",
      "",
      "### Option 1: Use designSystemWorkflow (Recommended)",
      "Trigger the `designSystemWorkflow` which guarantees skill activation:",
      "- The workflow activates the ui-ux-pro-max skill",
      "- Searches 7 design domains via BM25 (styles, colors, typography, charts, UX guidelines)",
      "- Returns a complete design system recommendation",
      "",
      "### Option 2: Delegate to Design Advisor",
      "Call `delegateToDesignAdvisor` for conversational design help.",
      "The Design Advisor has access to:",
      "- ui-ux-pro-max skill (795 lines of design intelligence)",
      "- getStyleRecommendations, getChartRecommendations, getTypographyRecommendations",
      "- getUXGuidelines, getProductRecommendations",
      "- searchDesignDatabase (BM25 RAG), generateDesignSystem",
      "",
      "## User Interaction",
      "- Present the design system recommendation to the user",
      "- If user wants alternatives, call the workflow/advisor again with their feedback",
      "- Show at most 2 options at a time to avoid overwhelming the user",
      "",
      "## PHASE ADVANCEMENT",
      "After the user approves a design system, IMMEDIATELY call `advancePhase` with:",
      "- nextPhase: 'build_preview'",
      "- selectedValue: the approved design system details",
      "",
      "## Do NOT",
      "- Do not show more than 2 style options at once",
