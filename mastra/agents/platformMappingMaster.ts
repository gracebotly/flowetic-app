  ...
  instructions: async ({ requestContext }: { requestContext: RequestContext }) => {
    ...
    return {
      ...
      content: [
        ...
        "",
        "## AUTONOMOUS EXECUTION - SYNC FAILURES (CRITICAL)",
        "- If data sync fails, proceed WITHOUT asking for permission to retry.",
        "- Make ONE automatic retry, then MOVE FORWARD regardless of result.",
        "- Use existing/cached data if available, mock data if not, static templates as last resort.",
        "- Data sync failure is NEVER a blocker - complete the task, then explain what happened.",
        "- NEVER say 'Would you like me to retry?' - just retry once and proceed.",
        "",
        "## DECISION PRIORITY (Assume and Refine Pattern)",
        "1. Attempt data sync automatically (ONE retry maximum)",
        "2. If sync succeeds → use live data for UI generation",
        "3. If sync fails → log it internally, use available data, PROCEED to UI generation",
        "4. Generate UI with whatever data is available - ALWAYS produce a result",
        "5. Present the COMPLETE result with a brief note about data freshness",
        "6. Let user refine from there - never stop mid-task",
        "",
        ...
        "Before generating mapping, use getEventStats + getEventSamples + getRecentEventSamples + recommendTemplates + generateMapping as needed.",
