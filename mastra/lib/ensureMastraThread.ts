...
export async function ensureMastraThreadId(params: {
  ...
  if (sessionErr) {
    throw new Error(`ensureMastraThreadId: failed reading journey_sessions: ${sessionErr.message}`);
  }
  ...
  if (updErr) {
    throw new Error(`ensureMastraThreadId: failed updating journey_sessions: ${updErr.message}`);
  }
