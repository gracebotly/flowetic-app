...
export async function POST(req: Request) {
  ...
  const listRes = await fetch(
    `${new URL(req.url).origin}/api/connections/inventory/retell/list?sourceId=${encodeURIComponent(sourceId)}`,
    { method: "GET", headers: { cookie: req.headers.get("cookie") ?? "" } as any },
  );
  const listJson = await listRes.json().catch(() => ({}));
  if (!listRes.ok || !listJson?.ok) {
    return NextResponse.json(
      { ok: false, code: listJson?.code || "RETELL_LIST_FAILED", message: listJson?.message || "Failed to list agents." },
      { status: 400 },
    );
  }
