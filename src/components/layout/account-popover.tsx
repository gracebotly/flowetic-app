...
function RowButton({
  ...
  return (
    ...
    >
      ...
      <span className="text-gray-500">{right ?? <ExternalLink size={16} className="opacity-0 group-hover:opacity-100 transition-opacity" />}</span>
    </button>
  )
}

export function AccountPopoverCard({
  email,
  plan = "Starter Plan",
  ...
  return (
    <Popover.Root>
      <Popover.Trigger asChild>
        ...
        >
          {/* The trigger visuals are rendered in the sidebar; this button just acts as trigger */}
        </button>
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Content
          ...
          align="start"
          sideOffset={12}
          className="z-50 w-80 rounded-xl border border-gray-200 bg-white p-0 shadow-xl outline-none"
        >
          {/* Header gradient */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-0.5 flex items-center gap-2 text-xs text-white/90">
                    ...
                    <button onClick={copyEmail} className="rounded bg-white/10 px-1.5 py-0.5 hover:bg-white/20">
                      <Copy size={14} />
                    </button>
          ...
          <Popover.Close className="absolute right-2 top-2 rounded bg-white/70 px-1 py-0.5 text-xs text-gray-700 hover:bg-white">Esc</Popover.Close>
        </Popover.Content>
      </Popover.Portal>
    </Popover.Root>
  )
