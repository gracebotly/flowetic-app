...
async function loadSkillMD(platformType: string, sourceId: string, entityId?: string) {
  ...
  useEffect(() => {
    const fetchDashboardSpec = async () => {
      ...
      try {
        // Server-side enrichment: event fetching, JSONB flattening, and
        // transformDataForComponents all happen on the server now.
        const res = await fetch(
          `/api/interfaces/${interfaceId}/preview-enriched?versionId=${versionId}`,
          { credentials: 'include' }
        );

        if (!res.ok) {
          console.error('[fetchDashboardSpec] Failed to fetch enriched spec:', res.status);
          hasFetchedSpecRef.current = false;
          return;
        }

        const data = await res.json();
        console.log('[fetchDashboardSpec] Loaded enriched spec:', {
          interfaceId,
          versionId,
