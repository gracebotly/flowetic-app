-- BUG 1 FIX: Add workflow_name parameter to scope entity discovery
DROP FUNCTION IF EXISTS get_data_driven_entities(UUID, UUID, INT);
...
BEGIN
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND (p_workflow_name IS NULL OR labels->>'workflow_name' = p_workflow_name);
  IF v_total = 0 OR v_total IS NULL THEN
    RETURN QUERY SELECT
      false::BOOLEAN,
      '[]'::JSONB,
      0::BIGINT,
      jsonb_build_object('earliest', NULL, 'latest', NULL);
    RETURN;
  END IF;
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  FROM (
      ...
      (
        ...
        FROM (
          ...
          WHERE sub.tenant_id = p_tenant_id
            ...
            AND sub.type NOT IN ('state', 'tool_event')
            AND (p_workflow_name IS NULL OR sub.labels->>'workflow_name' = p_workflow_name)
          LIMIT 100
        ) all_keys
      ) as distinct_fields
    ...
    WHERE e.tenant_id = p_tenant_id
      ...
      AND (p_workflow_name IS NULL OR e.labels->>'workflow_name' = p_workflow_name)
  ...
  RETURN QUERY SELECT
    ...
    v_total,
    jsonb_build_object('earliest', v_earliest, 'latest', v_latest);
END;
